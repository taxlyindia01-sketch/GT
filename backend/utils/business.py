...
#  P11 — calculate_tcs() always returns zero (TCS removed as a requirement)
...
# ── TCS — Section 206C(1F) — REMOVED (P11) ───────────────────
# TCS is no longer collected. calculate_tcs() is retained so existing
# code that imports it does not break; it always returns zero.

TCS_THRESHOLD = Decimal("500000")   # kept for reference only
TCS_RATE      = Decimal("0.01")     # kept for reference only

def calculate_tcs(subtotal: Decimal, pay_mode: str) -> dict:
    """
    TCS removed as of P11.
    Always returns zero. Signature unchanged for backward compatibility.
    """
    return {
