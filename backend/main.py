...
# ── Rate Limiter ──────────────────────────────────────────────
# Uses in-memory store by default; set REDIS_URL env var for Redis-backed
# limits that persist across dynos/restarts.
limiter = Limiter(
    key_func=get_remote_address,
    storage_uri=settings.REDIS_URL or "memory://",
    default_limits=[],        # no global default — we set per-endpoint only
)

# Attach limiter to auth router so @limiter.limit decorators work
auth.router.state.limiter = limiter
