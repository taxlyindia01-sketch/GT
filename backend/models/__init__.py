...
class Tenant(Base):
    ...
    logo_url:       Mapped[str|None] = mapped_column(Text)          # base64 or URL
    qr_code_url:    Mapped[str|None] = mapped_column(Text)          # UPI QR base64 or URL
    ...
    authorised_person: Mapped[str|None] = mapped_column(String(200))
...
# ── Credit Note ───────────────────────────────────────────────
# A Credit Note is the GST-compliant way to formally reverse an invoice.
# It preserves the original invoice number in the ledger (no gap),
# and creates a separate numbered document (CN-{tenant_id}-{id}).

class CreditNote(Base):
    """
    GST Credit Note — issued when a tax invoice needs to be reversed.

    Design:
      - The original Invoice remains in status=cancelled (visible in ledger)
      - This row holds the reversal document with its own sequential CN number
      - cash_receipts_fy on Customer is decremented here (not in cancel_invoice)
        so the FY reset and SFT logic stays consistent

    GST Rule 53(3): Credit note must be issued within the earlier of:
      (a) September 30 following the FY of the original supply, or
      (b) filing date of the annual return for that FY.
    """
    __tablename__ = "credit_notes"
    __table_args__ = (
        UniqueConstraint("tenant_id", "cn_no"),
        Index("ix_cn_tenant", "tenant_id"),
        Index("ix_cn_invoice", "original_invoice_id"),
    )

    id:                  Mapped[int]      = mapped_column(Integer, primary_key=True, autoincrement=True)
    tenant_id:           Mapped[int]      = mapped_column(ForeignKey("tenants.id", ondelete="CASCADE"))
    cn_no:               Mapped[str]      = mapped_column(String(30), nullable=False)   # CN-{tenant_id}-{id}
    cn_date:             Mapped[date]     = mapped_column(Date, nullable=False)
    original_invoice_id: Mapped[int]      = mapped_column(ForeignKey("invoices.id", ondelete="RESTRICT"))
    original_invoice_no: Mapped[str]      = mapped_column(String(30), nullable=False)   # denormalised for display
    customer_mobile:     Mapped[str]      = mapped_column(String(15), nullable=False)
    customer_name:       Mapped[str]      = mapped_column(String(200), nullable=False)
    customer_pan:        Mapped[str|None] = mapped_column(String(10))
    reason:              Mapped[str]      = mapped_column(String(500), nullable=False)   # mandatory for GST audit
    subtotal:            Mapped[Decimal]  = mapped_column(Numeric(15, 2), nullable=False)
    cgst:                Mapped[Decimal]  = mapped_column(Numeric(15, 2), default=0)
    sgst:                Mapped[Decimal]  = mapped_column(Numeric(15, 2), default=0)
    igst:                Mapped[Decimal]  = mapped_column(Numeric(15, 2), default=0)
    tcs_reversed:        Mapped[Decimal]  = mapped_column(Numeric(15, 2), default=0)
    total_reversed:      Mapped[Decimal]  = mapped_column(Numeric(15, 2), nullable=False)
    cash_fy_reversed:    Mapped[Decimal]  = mapped_column(Numeric(15, 2), default=0)   # how much we subtracted from cash_receipts_fy
    created_by:          Mapped[int|None] = mapped_column(ForeignKey("users.id"))
    created_at:          Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow)

    original_invoice: Mapped[Invoice] = relationship(foreign_keys=[original_invoice_id])


# ── FY Reset Audit Log ────────────────────────────────────────

class FYResetLog(Base):
    """Immutable audit log — one row per FY reset run."""
    __tablename__ = "fy_reset_log"

    id:           Mapped[int]      = mapped_column(Integer, primary_key=True, autoincrement=True)
    reset_date:   Mapped[date]     = mapped_column(Date, nullable=False)
    rows_updated: Mapped[int]      = mapped_column(Integer, nullable=False)
    triggered_by: Mapped[str]      = mapped_column(String(100), nullable=False)   # "fy_reset.py cron" or username
    created_at:   Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow)
