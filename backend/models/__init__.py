...
class Supplier(Base):
    ...
    invoices: Mapped[list["SupplierInvoice"]] = relationship(back_populates="supplier", cascade="all, delete-orphan",
    ...
    advances: Mapped[list["SupplierAdvance"]] = relationship(back_populates="supplier", cascade="all, delete-orphan",
...
class SupplierInvoice(Base):
    ...
    supplier: Mapped["Supplier"]                   = relationship(back_populates="invoices",
                                                       ...
                                                       foreign_keys="[SupplierInvoice.tenant_id, SupplierInvoice.supplier_mobile]")
    items:    Mapped[list["SupplierInvoiceItem"]]  = relationship(back_populates="invoice", cascade="all, delete-orphan")
...
class SupplierPayment(Base):
    ...
    tenant_id:       Mapped[int]      = mapped_column(ForeignKey("tenants.id", ondelete="CASCADE"))
    supplier_mobile: Mapped[str]      = mapped_column(String(15), nullable=False)
    invoice_id:      Mapped[int|None] = mapped_column(ForeignKey("supplier_invoices.id"))
    amount:          Mapped[Decimal]  = mapped_column(Numeric(15, 2), nullable=False)
    payment_date:    Mapped[date]     = mapped_column(Date, nullable=False)
    ...
    supplier: Mapped["Supplier"]        = relationship(back_populates="payments",
                                             ...
                                             foreign_keys="[SupplierPayment.tenant_id, SupplierPayment.supplier_mobile]")
    invoice:  Mapped["SupplierInvoice|None"] = relationship(back_populates="payments",
                                                 foreign_keys="[SupplierPayment.invoice_id]")
...
class SupplierAdvance(Base):
    ...
    supplier: Mapped["Supplier"] = relationship(back_populates="advances",
