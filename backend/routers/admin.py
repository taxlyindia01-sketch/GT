...
# ── Import your existing admin config ────────────────────────────────────────
# These should already be defined in your project — adjust import path as needed
try:
    from utils.admin_auth import (
        ADMIN_USERNAME, ADMIN_PASSWORD_HASH, ADMIN_SECRET_KEY, get_admin_payload
    )
except ImportError:
    # Fallback: define inline if not available separately
    import os
    ADMIN_USERNAME     = os.getenv("ADMIN_USERNAME", "superadmin")
    ADMIN_PASSWORD_HASH = os.getenv("ADMIN_PASSWORD_HASH", "")
    ADMIN_SECRET_KEY   = os.getenv("ADMIN_SECRET_KEY", os.getenv("SECRET_KEY", "changeme"))
    ...
    async def get_admin_payload(
        ...
        try:
            return jwt.decode(creds.credentials, ADMIN_SECRET_KEY, algorithms=["HS256"])
        except Exception:
