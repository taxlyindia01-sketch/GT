...
async def record_payment(
    ...
    # Resolve customer name (Issue 11 — use provided or fetch from master)
    customer_name = body.customer_name or invoice.customer_name

    payment = Payment(
        tenant_id=tenant_id,
        invoice_id=body.invoice_id,
        customer_mobile=body.customer_mobile,
        customer_name=customer_name,        # Issue 11
        amount=body.amount,
    ...
    if body.pay_mode == "Cash":
        ...
        db.add(CashEntry(
            ...
            amount=body.amount,
            description=f"Payment — {customer_name} ({invoice.invoice_no})",
            invoice_id=body.invoice_id,
            created_by=int(payload["sub"]),
        ))
...
async def list_payments(
    ...
    for p in payments:
        ...
        cname = p.customer_name if hasattr(p, "customer_name") and p.customer_name else None
        if not cname:
