...
from limiter import limiter   # ← shared instance (fixes HTTP 500 on login)
...
class DemoSignupRequest(BaseModel):
    ...
    def password_strength(cls, v: str) -> str:
        """Password must contain ≥1 digit AND ≥1 special character."""
        import re
...
async def password_login(
    ...
    # Check approval status — give useful error messages
    ...
    # Compute trial info for token payload and response
    is_trial = user.approval_status == ApprovalStatus.trial
    days_left = trial_days_remaining(user.trial_expires_at) if is_trial else 0
    ...
    token = create_access_token({
        ...
        "mobile":       user.mobile,
        "trial_active": is_trial,        # ← was missing; caused frontend state bug
    })
...
async def google_login(
    ...
    if is_trial and 0 < days_left <= 3 and user.email:
        import asyncio
        asyncio.create_task(
...
async def google_signup(body: GoogleSignupRequest, db: AsyncSession = Depends(get_db)):
    ...
    if email:
        import asyncio
        asyncio.create_task(
            send_welcome_email(email, name, body.company_name, settings.TRIAL_DAYS)
        )
