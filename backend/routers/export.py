...
async def export_all_reports_excel(
    ...
    threshold = Decimal("200000")
    v269_stmt = select(Payment).where(
        Payment.tenant_id == tid,
        Payment.pay_mode  == "Cash",
        Payment.amount    >= threshold,
    )
    if from_date: v269_stmt = v269_stmt.where(Payment.payment_date >= from_date)
    if to_date:   v269_stmt = v269_stmt.where(Payment.payment_date <= to_date)
    v269_result = await db.execute(v269_stmt.order_by(Payment.payment_date.desc()))
    viol_pays   = v269_result.scalars().all()
    ...
    for p in viol_pays:
        ...
        viol_rows.append([
            p.payment_date.isoformat(),
            inv_obj.invoice_no if inv_obj else "—",
            inv_obj.customer_name if inv_obj else (cust.name if cust else "—"),
            p.customer_mobile,
            cust.pan if cust else "MISSING",
            float(p.amount), float(p.amount), p.reference_no or ""
        ])
    add_sheet(wb, "Sec 269ST Violations", [
        "Date","Invoice No","Customer","Mobile","PAN","Cash Amount","Penalty Risk","Reference"
    ], viol_rows)
