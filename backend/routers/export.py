...
async def export_all_reports_excel(
    ...
    # Add Account Register sheet (Fix: was missing from all-reports-excel)
    await add_account_sheet(wb, db, tid, from_date, to_date)
    ...
    # ── Sheet: Cancelled Invoices ────────────────────────────
    cancel_stmt = (
        select(Invoice)
        .where(Invoice.tenant_id == tid, Invoice.status == "cancelled")
        .order_by(Invoice.updated_at.desc())
    )
    if from_date: cancel_stmt = cancel_stmt.where(Invoice.invoice_date >= from_date)
    if to_date:   cancel_stmt = cancel_stmt.where(Invoice.invoice_date <= to_date)
    cancel_result  = await db.execute(cancel_stmt)
    cancelled_invs = cancel_result.scalars().all()
    add_sheet(wb, "Cancelled Invoices", [
        "Invoice No", "Invoice Date", "Cancelled At", "Customer",
        "Mobile", "PAN", "Pay Mode", "Grand Total", "Notes"
    ], [
        [inv.invoice_no, inv.invoice_date.isoformat(),
         inv.updated_at.isoformat() if inv.updated_at else "",
         inv.customer_name, inv.customer_mobile, inv.customer_pan or "",
         inv.pay_mode.value, float(inv.grand_total), inv.notes or ""]
...
async def export_cancelled_invoices_excel(
    ...
    """
    Export Cancelled Invoices register to Excel.
    ...
    headers = [
        "Invoice No", "Invoice Date", "Cancelled At",
        "Customer", "Mobile", "PAN", "Pay Mode",
        "Subtotal", "GST", "Grand Total", "Notes"
    ]
    ws.append(headers)
    style_header_row(ws, 1, len(headers))
    ...
    for inv in invoices:
        ws.append([
            ...
            inv.pay_mode.value,
            float(inv.subtotal),
            float(inv.cgst + inv.sgst + inv.igst),
            float(inv.grand_total),
            inv.notes or "",
        ])
    auto_col_width(ws)
