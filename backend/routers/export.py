...
async def export_supplier_all_excel(
    ...
    from models import (Supplier, SupplierInvoice, SupplierInvoiceItem,
                        SupplierPayment, SupplierAdvance, Tenant)
    from decimal import Decimal
    ...
    adv_remaining    = sum(float(a.remaining_amount if hasattr(a,"remaining_amount") else a.amount) for a in sup_advs)
    ...
    biz_name = "GoldTrader Pro"
    try:
        tn_r = await db.execute(select(Tenant).where(Tenant.id == tid).limit(1))
        tn   = tn_r.scalar_one_or_none()
        if tn and tn.company_name:
            biz_name = tn.company_name
    except Exception:
        pass

    # Title row — Business Name
    ws_dash.merge_cells("A1:F1")
    dash_cell(1, 1, f"🏭  {biz_name.upper()}  —  SUPPLIER DASHBOARD",
              font=title_font, fill=dark_fill, align="center", border=False)
    ws_dash.row_dimensions[1].height = 36

    # Row 2 — Date Range
    ws_dash.merge_cells("A2:F2")
    period_str = f"Period: {from_date.strftime('%d %b %Y') if from_date else 'All time'}  →  {to_date.strftime('%d %b %Y') if to_date else today_dt.strftime('%d %b %Y')}"
    pc = ws_dash.cell(2, 1, period_str)
    pc.font      = Font(name="Calibri", color="CCCCCC", size=10, italic=True)
    pc.fill      = PatternFill("solid", fgColor="0F0F1E")
    pc.alignment = Alignment(horizontal="center", vertical="center")
    ws_dash.row_dimensions[2].height = 18

    # KPI headers row 3 (was row 3 → still row 3, blank row 2 now used for period)
    ...
    # "Generated by" footer row
    footer_row = guide_row + len(sheet_guide) + 2
    ws_dash.merge_cells(f"A{footer_row}:F{footer_row}")
    fc = ws_dash.cell(footer_row, 1, "📊  Report generated through GoldTrader Pro by Taxly India  |  taxlyindia.com")
    fc.font      = Font(name="Calibri", color="888888", size=9, italic=True)
    fc.fill      = PatternFill("solid", fgColor="0D0D1A")
    fc.alignment = Alignment(horizontal="center", vertical="center")
    ws_dash.row_dimensions[footer_row].height = 16
    ...
    add_sheet(wb, "Payments",
        ...
        [[p.payment_date.isoformat(),
          ...
          p.supplier_mobile, float(p.amount), p.payment_mode.value if hasattr(p.payment_mode,"value") else str(p.payment_mode),
          p.reference_number or "", p.notes or ""]
         for p in sup_pays])
    ...
    add_sheet(wb, "Advances",
        ...
        [[a.advance_date.isoformat(),
          ...
          float(a.remaining_amount if hasattr(a,"remaining_amount") else a.amount),
          a.payment_mode.value if hasattr(a.payment_mode,"value") else str(a.payment_mode),
          a.notes or ""]
         for a in sup_advs])
