...
async def add_dashboard_sheet(
    ...
    No borders, no gridlines, well-coloured, professional presentation.
    """
    ...
    TITLE_FONT  = Font(name="Calibri", bold=True, size=18, color=C_NAV)
    SUB_FONT    = Font(name="Calibri", size=10, color=C_MUTED)
    HDR_FONT    = Font(name="Calibri", bold=True, size=11, color=C_WHT)
    LBL_FONT    = Font(name="Calibri", bold=True, size=10, color=C_NAV)
    VAL_FONT    = Font(name="Calibri", size=10, color=C_BODY)
    VAL_BOLD    = Font(name="Calibri", bold=True, size=11, color=C_GOLD)
    SMALL_FONT  = Font(name="Calibri", size=9,  color=C_MUTED)
    ...
    def sc(r, c, v, font=None, fill=None, al=None, fmt=None, border=None):
        ...
        cell.border = border or NO_BORDER
        return cell
    ...
    paid_list    = [i for i in invoices if i.payment_status.value == "paid"]
    partial_list = [i for i in invoices if i.payment_status.value == "partial"]
    unpaid_list  = [i for i in invoices if i.payment_status.value == "unpaid"]
    violations   = [i for i in invoices if i.pay_mode.value == "Cash" and float(i.grand_total) >= 200000]
    ...
    # ── Set column widths ─────────────────────────────────────────
    col_widths = {
        "A": 3, "B": 28, "C": 20, "D": 3, "E": 28, "F": 20, "G": 3,
        "H": 24, "I": 18, "J": 3,
    }
    ...
    sc(1, 2, "GOLDTRADER PRO — BUSINESS DASHBOARD",
       font=Font(name="Calibri", bold=True, size=20, color=C_WHT),
       fill=fill_solid(C_NAV), al=_C)
    ws.row_dimensions[1].height = 32
    ws.row_dimensions[2].height = 0

    ws.merge_cells("B3:I3")
    sc(3, 2, f"Period: {period_label}  |  Generated: {today_dt.strftime('%d %b %Y')}",
       font=SUB_FONT, fill=fill_solid("F5F7FA"), al=_C)
    ws.row_dimensions[3].height = 16

    # ── Row 5-8: KPI boxes (2 columns of 3) ──────────────────────
    ws.row_dimensions[4].height = 10  # spacer

    def kpi_box(row, col_start, label, value, bg, fg_val, fmt_val):
        """Draw a KPI card spanning 2 columns and 3 rows"""
        end_col = col_start + 1
        ws.merge_cells(
            start_row=row, start_column=col_start,
            end_row=row, end_column=end_col
        )
        sc(row, col_start, label,
           font=Font(name="Calibri", size=9, color="999999"),
           fill=fill_solid(bg), al=_C)
        ws.merge_cells(
            start_row=row+1, start_column=col_start,
            end_row=row+1, end_column=end_col
        )
        sc(row+1, col_start, value,
           font=Font(name="Calibri", bold=True, size=14, color=fg_val),
           fill=fill_solid(bg), al=_C,
           fmt=fmt_val)
        # bottom filler row
        for c in range(col_start, end_col+1):
            sc(row+2, c, "", fill=fill_solid(bg))
        for rr in [row, row+1, row+2]:
            ws.row_dimensions[rr].height = 20

    kpi_box(5,  2, "Total Invoices",      n_invoices,    "EEF2FF", C_NAV, "0")
    kpi_box(5,  5, "Grand Total Sales",   total_grand,   C_GOLD_LT, C_GOLD, '#,##0.00')
    kpi_box(5,  8, "Total GST",           total_gst,     "E8EEF6", C_NAV, '#,##0.00')
    kpi_box(9,  2, "Amount Collected",    total_paid,    C_GRN_LT, C_GRN, '#,##0.00')
    kpi_box(9,  5, "Outstanding Balance", total_out,     C_AMB_LT, C_AMB, '#,##0.00')
    kpi_box(9,  8, "269ST Violations",    len(violations), C_RED_LT, C_RED, "0")

    ws.row_dimensions[8].height = 8   # spacer between kpi rows
    ws.row_dimensions[12].height = 12  # spacer after kpis

    # ── Row 13-14: Section headers ────────────────────────────────
    def section_hdr(row, col, text, span=2):
        ws.merge_cells(start_row=row, start_column=col, end_row=row, end_column=col+span-1)
        sc(row, col, text, font=HDR_FONT, fill=fill_solid(C_NAV), al=_L)
        ws.row_dimensions[row].height = 22

    section_hdr(13, 2, "FINANCIAL SUMMARY", 2)
    section_hdr(13, 5, "INVOICE STATUS", 2)

    # Financial summary table (col B-C)
    fin_data = [
        ("Taxable Value",   total_subtotal),
    ...
    fin_bgs = [C_WHT, "F8F8F8"] * 5
    for idx, (lbl, val) in enumerate(fin_data):
        r = 14 + idx
        bg = fin_bgs[idx % 2]
        sc(r, 2, lbl, font=LBL_FONT, fill=fill_solid(bg), al=_L)
        sc(r, 3, val, font=Font(name="Calibri", bold=(lbl in ("Grand Total", "Outstanding", "Collected")),
                                size=10, color=(C_GRN if lbl=="Collected" else C_AMB if lbl=="Outstanding" else C_BODY)),
           fill=fill_solid(bg), al=_R, fmt='#,##0.00')
        ws.row_dimensions[r].height = 18

    # Invoice status table (col E-F)
    status_data = [
        ("Fully Paid",      len(paid_list),    C_GRN),
        ("Partially Paid",  len(partial_list), C_AMB),
        ("Unpaid",          len(unpaid_list),  C_RED),
        ("Total",           n_invoices,        C_NAV),
    ]
    for idx, (lbl, val, clr) in enumerate(status_data):
        r = 14 + idx
        bg = fin_bgs[idx % 2]
        sc(r, 5, lbl, font=LBL_FONT, fill=fill_solid(bg), al=_L)
        sc(r, 6, val, font=Font(name="Calibri", bold=True, size=12, color=clr),
           fill=fill_solid(bg), al=_C)
        ws.row_dimensions[r].height = 18

    # ── Payment mode data for chart ───────────────────────────────
    ws.row_dimensions[23].height = 14

    section_hdr(24, 2, "SALES BY PAYMENT MODE", 2)
    section_hdr(24, 5, "SALES BY CATEGORY", 2)

    # Payment mode table
    for idx, (mode, amt) in enumerate(mode_sorted):
        r = 25 + idx
        bg = fin_bgs[idx % 2]
        sc(r, 2, mode, font=LBL_FONT, fill=fill_solid(bg), al=_L)
        sc(r, 3, amt,  font=VAL_FONT,  fill=fill_solid(bg), al=_R, fmt='#,##0.00')
        ws.row_dimensions[r].height = 18
    ...
    for idx, (cat, amt) in enumerate(cat_sorted):
        r = 25 + idx
        bg = fin_bgs[idx % 2]
        sc(r, 5, cat, font=LBL_FONT, fill=fill_solid(bg), al=_L)
        sc(r, 6, amt, font=VAL_FONT,  fill=fill_solid(bg), al=_R, fmt='#,##0.00')
        ws.row_dimensions[r].height = 18
    ...
    chart_row_start = max(25 + mode_count, 25 + cat_count) + 2
    ...
    if mode_count >= 1:
        ...
        # Series: amounts in col C, labels in col B
        data_ref = Reference(ws, min_col=3, min_row=chart_data_start_row,
                             max_row=chart_data_start_row + mode_count - 1)
        cats_ref = Reference(ws, min_col=2, min_row=chart_data_start_row,
                             max_row=chart_data_start_row + mode_count - 1)
        pie.add_data(data_ref)
        pie.set_categories(cats_ref)
        pie.series[0].title = None
        # Colour slices
        slice_colors = ["C8900A", "1F3864", "1A6B3C", "B71C1C", "92400E", "607D8B"]
        for si, hex_c in enumerate(slice_colors[:mode_count]):
            dp = DataPoint(idx=si)
            dp.graphicalProperties.solidFill = hex_c
            pie.series[0].dPt.append(dp)
        ws.add_chart(pie, f"B{chart_row_start}")
    ...
    if cat_count >= 1:
        ...
        data_ref2 = Reference(ws, min_col=6, min_row=25,
                              max_row=25 + cat_count - 1)
        cats_ref2 = Reference(ws, min_col=5, min_row=25,
                              max_row=25 + cat_count - 1)
        bar.add_data(data_ref2)
        bar.set_categories(cats_ref2)
        bar.series[0].title = None
        # Gold colour for bars
        bar.series[0].graphicalProperties.solidFill = "C8900A"
        bar.series[0].graphicalProperties.line.solidFill = "9B6A00"
        ws.add_chart(bar, f"E{chart_row_start}")

    # ── Footer ────────────────────────────────────────────────────
    footer_row = chart_row_start + 18
    ws.merge_cells(f"B{footer_row}:I{footer_row}")
    sc(footer_row, 2,
       "GoldTrader Pro — Jewellery CRM & Invoicing | Taxly India",
       font=Font(name="Calibri", size=9, color=C_MUTED, italic=True),
       fill=fill_solid("F5F7FA"), al=_C)
