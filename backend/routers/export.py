...
async def export_all_reports_excel(
    ...
    # ── Sheet 0: Business Dashboard ──────────────────────────
    ws_d = wb.create_sheet("Dashboard", 0)
    _d_dark  = PatternFill("solid", fgColor="1A1A2E")
    _d_gold  = PatternFill("solid", fgColor="B8860B")
    _d_grn   = PatternFill("solid", fgColor="1E6B2E")
    _d_red   = PatternFill("solid", fgColor="8B0000")
    _d_blue  = PatternFill("solid", fgColor="0D3B66")
    _d_mid   = PatternFill("solid", fgColor="2B2B4B")
    _d_ft    = Font(name="Calibri", bold=True, color="FFD700", size=16)
    _d_lbl   = Font(name="Calibri", bold=True, color="CCCCCC", size=10)
    _d_val   = Font(name="Calibri", bold=True, color="FFFFFF", size=13)
    _d_hdr   = Font(name="Calibri", bold=True, color="FFD700", size=11)
    _d_sub   = Font(name="Calibri", bold=False, color="AAAAAA", size=9)
    _d_cnt   = Font(name="Calibri", bold=True, color="FFD700", size=10)
    _ctr     = Alignment(horizontal="center", vertical="center")
    _lft     = Alignment(horizontal="left",   vertical="center")
    _rgt     = Alignment(horizontal="right",  vertical="center", wrap_text=False)

    def _dc(row, col, val, font=None, fill=None, align=None, num_fmt=None):
        c = ws_d.cell(row=row, column=col, value=val)
        if font:    c.font    = font
        if fill:    c.fill    = fill
    ...
    period_txt = f"Period: {from_date.strftime('%d %b %Y') if from_date else 'All time'} → {to_date.strftime('%d %b %Y') if to_date else per}"

    # Title row
    ws_d.merge_cells("A1:H1")
    _dc(1,1,"🏆  GOLDTRADER PRO — COMPLETE BUSINESS DASHBOARD",font=_d_ft,fill=_d_dark,align=_ctr)
    ws_d.row_dimensions[1].height = 40

    ws_d.merge_cells("A2:H2")
    _dc(2,1,period_txt,font=Font(name="Calibri",bold=False,color="AAAAAA",size=10),fill=_d_dark,align=_ctr)
    ws_d.row_dimensions[2].height = 20

    # ── Customer block (col A-D) ──────────────────────────────
    ws_d.merge_cells("A4:D4")
    _dc(4,1,"👥  CUSTOMER (SALES) SUMMARY",font=_d_hdr,fill=_d_blue,align=_ctr)
    ws_d.row_dimensions[4].height = 22

    c_kpi_labels = ["Total Customers","Sales Invoices","Total Sales (₹)","Total Collected (₹)","Receivables (₹)","Cash Sales (₹)"]
    c_kpi_values = [len(cust_list), c_inv_count, c_total_inv, c_total_paid, c_outstanding, c_cash_sales]
    for i,(lbl,val) in enumerate(zip(c_kpi_labels, c_kpi_values)):
        r = 5 + i
        ws_d.merge_cells(f"A{r}:B{r}")
        _dc(r,1,lbl,font=_d_lbl,fill=_d_mid,align=_lft)
        ws_d.merge_cells(f"C{r}:D{r}")
        fmt = "#,##0" if isinstance(val,int) else "#,##0.00"
        _dc(r,3,val,font=_d_val,fill=_d_dark,align=_rgt,num_fmt=fmt)
        ws_d.row_dimensions[r].height = 20

    # ── Supplier block (col E-H) ──────────────────────────────
    ws_d.merge_cells("E4:H4")
    _dc(4,5,"🏭  SUPPLIER (PURCHASE) SUMMARY",font=_d_hdr,fill=PatternFill("solid",fgColor="4A1E1E"),align=_ctr)

    s_kpi_labels = ["Total Suppliers","Purchase Invoices","Total Purchases (₹)","Total Paid (₹)","Payables (₹)","Advances Given (₹)"]
    s_kpi_values = [len(sup_list), len(s_invs), s_total_inv, s_total_paid, s_outstanding, s_total_advs]
    for i,(lbl,val) in enumerate(zip(s_kpi_labels, s_kpi_values)):
        r = 5 + i
        ws_d.merge_cells(f"E{r}:F{r}")
        _dc(r,5,lbl,font=_d_lbl,fill=_d_mid,align=_lft)
        ws_d.merge_cells(f"G{r}:H{r}")
        fmt = "#,##0" if isinstance(val,int) else "#,##0.00"
        _dc(r,7,val,font=_d_val,fill=_d_dark,align=_rgt,num_fmt=fmt)
        ws_d.row_dimensions[r].height = 20

    # ── Divider ───────────────────────────────────────────────
    ws_d.merge_cells("A12:H12")
    _dc(12,1,"",fill=_d_gold)
    ws_d.row_dimensions[12].height = 4

    # ── Top 5 Customers by Outstanding ───────────────────────
    ws_d.merge_cells("A13:D13")
    _dc(13,1,"📊  TOP CUSTOMERS — RECEIVABLES",font=_d_hdr,fill=_d_blue,align=_ctr)
    ws_d.row_dimensions[13].height = 20
    ...
    for i, x in enumerate(top_c):
        r = 14 + i
        ws_d.merge_cells(f"A{r}:B{r}")
        _dc(r,1,x["name"],font=_d_lbl,fill=_d_mid,align=_lft)
        ws_d.merge_cells(f"C{r}:D{r}")
        _dc(r,3,x["out"],font=_d_val,fill=_d_dark,align=_rgt,num_fmt="#,##0.00")
        ws_d.row_dimensions[r].height = 18

    # ── Top 5 Suppliers by Outstanding ───────────────────────
    ws_d.merge_cells("E13:H13")
    _dc(13,5,"📊  TOP SUPPLIERS — PAYABLES",font=_d_hdr,fill=PatternFill("solid",fgColor="4A1E1E"),align=_ctr)
    ...
    for i, x in enumerate(top_s):
        r = 14 + i
        ws_d.merge_cells(f"E{r}:F{r}")
        _dc(r,5,x["name"],font=_d_lbl,fill=_d_mid,align=_lft)
        ws_d.merge_cells(f"G{r}:H{r}")
        _dc(r,7,x["out"],font=_d_val,fill=_d_dark,align=_rgt,num_fmt="#,##0.00")
        ws_d.row_dimensions[r].height = 18

    # ── Footer ────────────────────────────────────────────────
    ws_d.merge_cells("A20:H20")
    _dc(20,1,f"Generated on {per} via GoldTrader Pro by Taxly India | taxlyindia.com",
        font=Font(name="Calibri",bold=False,color="777777",size=8),fill=_d_dark,align=_ctr)
    ws_d.row_dimensions[20].height = 16
    ...
    if "Account Register" in wb.sheetnames:
        wb.move_sheet("Account Register", offset=-len(wb.sheetnames)+2)
    # ── POINT 4: Combined Customer + Supplier Dashboard as Sheet 1 ──
    from models import SupplierInvoice, SupplierPayment, SupplierAdvance, Supplier
    ...
    sup_invs = sup_inv_r.scalars().all()
    if from_date:
        sup_invs = [s for s in sup_invs if s.invoice_date >= from_date]
    if to_date:
        sup_invs = [s for s in sup_invs if s.invoice_date <= to_date]

    sup_cnt_r = await db.execute(
        select(func.count(Supplier.mobile)).where(Supplier.tenant_id == tid)
    )
    total_suppliers = sup_cnt_r.scalar() or 0

    sup_invoiced    = sum(float(i.grand_total)   for i in sup_invs)
    sup_paid        = sum(float(i.amount_paid)   for i in sup_invs)
    sup_outstanding = sum(float(i.outstanding)   for i in sup_invs)

    # Create combined dashboard sheet
    ws_dash = wb.create_sheet("Dashboard")
    wb.move_sheet("Dashboard", offset=-len(wb.sheetnames) + 1)

    def _dc(row, col, value, bold=False, color="000000", bg=None, align="left", size=10):
        c = ws_dash.cell(row=row, column=col, value=value)
        c.font = Font(name="Calibri", bold=bold, color=color, size=size)
        if bg: c.fill = PatternFill("solid", fgColor=bg)
        c.alignment = _Align(horizontal=align, vertical="center")
    ...
    t = ws_dash.cell(1, 1, f"📊  GOLDTRADER PRO — COMBINED REPORT  ·  {today_dt.strftime('%d %b %Y')}")
    t.font = Font(name="Calibri", bold=True, color="FFD700", size=16)
    t.fill = PatternFill("solid", fgColor="0D0D1A")
    t.alignment = _Align(horizontal="center", vertical="center")
    ws_dash.row_dimensions[1].height = 36

    ws_dash.merge_cells("A2:H2")
    period = f"Period: {from_date or 'All time'} → {to_date or today_dt.isoformat()}"
    p = ws_dash.cell(2, 1, period)
    p.font = Font(name="Calibri", color="AAAAAA", size=10, italic=True)
    p.fill = PatternFill("solid", fgColor="0D0D1A")
    p.alignment = _Align(horizontal="center")
    ...
    cust_color = "1A3A5C"; sup_color = "3A1A5C"
    _dc(4, 1, "CUSTOMER SUMMARY", bold=True, color="FFFFFF", bg=cust_color, align="center", size=11)
    ws_dash.merge_cells("A4:D4")
    _dc(4, 5, "SUPPLIER SUMMARY", bold=True, color="FFFFFF", bg=sup_color, align="center", size=11)
    ws_dash.merge_cells("E4:H4")

    # Customer KPI labels + values (row 5 = labels, row 6 = values)
    cust_kpis = [
        ("Total Invoices", len(invoices)),
        ("Total Sales", f"₹{sum(float(i.grand_total) for i in invoices):,.2f}"),
        ("Total GST", f"₹{sum(float(i.cgst)+float(i.sgst)+float(i.igst) for i in invoices):,.2f}"),
        ("Outstanding", f"₹{sum(float(i.outstanding) for i in invoices if float(i.outstanding) > 0):,.2f}"),
    ]
    sup_kpis = [
        ("Total Suppliers", total_suppliers),
        ("Total Purchased", f"₹{sup_invoiced:,.2f}"),
        ("Amount Paid", f"₹{sup_paid:,.2f}"),
        ("Outstanding", f"₹{sup_outstanding:,.2f}"),
    ]
    ws_dash.row_dimensions[5].height = 18
    ws_dash.row_dimensions[6].height = 26
    for col, (lbl, val) in enumerate(cust_kpis, 1):
        _dc(5, col, lbl, bold=True, color="AAAAAA", bg="0D1A2E", align="center", size=9)
        _dc(6, col, val, bold=True, color="FFD700", bg=cust_color, align="center", size=12)
    for col, (lbl, val) in enumerate(sup_kpis, 1):
        _dc(5, col + 4, lbl, bold=True, color="AAAAAA", bg="1A0D2E", align="center", size=9)
        _dc(6, col + 4, val, bold=True, color="FFD700", bg=sup_color, align="center", size=12)

    # Sheet guide
    ws_dash.row_dimensions[7].height = 10
    guide = [
        ("Sheet", "Contents"),
        ("Sales Register",       "All customer invoices with GST breakdown"),
        ("GSTR-1",               "Customer GST output register"),
        ("Outstanding",          "Customer invoices with pending balances"),
        ("Payments",             "Customer payments received"),
        ("Item-wise",            "Category-wise sales summary"),
        ("SFT Register",         "Section 206 SFT compliance data"),
        ("Sec 269ST Violations", "Cash receipt limit violations"),
        ("Cash Book",            "Cash inflow/outflow register"),
        ("Supplier Invoices",    "All purchase invoices with GST"),
        ("Supplier Payments",    "Supplier payments made"),
        ("Supplier Advances",    "Advances paid to suppliers"),
        ("Supplier Outstanding", "Purchase invoices with pending balances"),
        ("GSTR-2B Purchase",     "Supplier input GST reconciliation"),
    ]
    for i, (sheet, desc) in enumerate(guide):
        row = 8 + i
        is_hdr = i == 0
        bg     = "0D0D1A" if not is_hdr else "333355"
        _dc(row, 1, sheet, bold=is_hdr, color="FFD700" if not is_hdr else "FFFFFF",
            bg=bg, align="left", size=10)
        ws_dash.merge_cells(f"B{row}:H{row}")
        _dc(row, 2, desc, color="CCCCCC" if not is_hdr else "FFFFFF", bg=bg, size=9)
        ws_dash.row_dimensions[row].height = 17

    for col, w in enumerate([22, 42, 14, 14, 22, 14, 14, 14], 1):
        ws_dash.column_dimensions[_gcl(col)].width = w
