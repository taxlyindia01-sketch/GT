...
async def export_fifo_excel(
    ...
    """
    FIFO Excel — 3 sheets with correct live-replay logic.
    Sheet 1 · Stock Movement  : every deduped transaction, value_out = FIFO COGS
    Sheet 2 · Category Summary: qty/value IN, COGS, qty on hand, closing value
    Sheet 3 · FIFO Valuation  : per-item closing batches + avg rate
    """
    ...
    def _dedup(txns):
        """Keep latest txn per (invoice_id, direction); keep all txns without invoice."""
        seen = {}; no_inv = []
        for t in txns:
            if t.invoice_id is None:
                no_inv.append(t)
            else:
                d = "IN" if t.qty > 0 else "OUT"
                k = (t.invoice_id, d)
                if k not in seen or t.id > seen[k].id:
                    seen[k] = t
    ...
    # ── Per-item replay ─────────────────────────────────────────
    move_rows = []   # for Sheet 1
    cat_map   = {}   # for Sheet 2
    val_rows  = []   # for Sheet 3
    ...
    for stock in stocks:
        ...
        # Always load ALL txns from beginning (no from_date filter here!)
        all_txns_r = await db.execute(
        ...
        raw_txns = all_txns_r.scalars().all()
        txns     = _dedup(raw_txns)
        ...
        for t in txns:
            ...
            # Skip OUT rows for cancelled invoices
            if qty < 0 and t.invoice_id and t.invoice_id in cancelled_ids:
                continue
            ...
            if qty > 0:
                ...
                row_val_in  = 0.0
                row_val_out = float(cogs)
                qty_out += qty_abs
                val_out += cogs

            # Only add to movement sheet within date window
            ...
            if in_window:
                move_rows.append([
                    ...
                    float(qty_abs),
                    float(rate) if qty > 0 else 0.0,
                    round(row_val_in,  2),
                    round(row_val_out, 2),
                ])

        closing_batches = [b for b in fifo_batches if b["qty_remaining"] > 0]
        closing_value   = sum(b["qty_remaining"] * b["purchase_rate"] for b in closing_batches)
        qty_on_hand     = float(qty_in - qty_out)
        avg_rate        = float(closing_value / (qty_in - qty_out)) if (qty_in - qty_out) > 0 else 0.0
        tv              = float(closing_value)

        val_rows.append([cat, stock.purity or "—", stock.description, unit,
                         float(qty_in), float(qty_out), round(qty_on_hand, 4),
                         round(avg_rate, 2), round(tv, 2)])

        if cat not in cat_map:
            cat_map[cat] = [Decimal("0")] * 5  # in, out, on_hand, val_in, val_out, closing
        ...
        cat_map[cat]["closing"] += closing_value
    ...
    # ════════════════════════════════════════════════════════════
    # SHEET 1 — Invoice-wise Stock Movement
    # ════════════════════════════════════════════════════════════
    ws1 = wb.create_sheet("Stock Movement")
    title_block(ws1, "INVOICE-WISE STOCK MOVEMENT REGISTER",
                f"Period: {period}  |  Generated: {today_str}  |  Value OUT = FIFO COGS", 13)
    hdr_row(ws1, 4, [
        "Date", "Direction", "Txn Type", "Invoice No", "Party",
        "Category", "Purity", "Description", "Unit",
        "Qty", "Purchase Rate (₹)", "Value IN (₹)", "Value OUT / COGS (₹)"
    ])

    t_qi = 0.0; t_vi = 0.0; t_vo = 0.0
    for i, row in enumerate(move_rows):
        ...
        sc(ws1, r, 6,  row[5],  font=Font(name="Calibri", bold=True,
                                           color=CAT_CLR.get(row[5], "333333"), size=10),
           fill=rf, al=_C)
        ...
        sc(ws1, r, 11, row[10] or "", font=T_IN,  fill=rf, al=_R,
           fmt="#,##0.00" if row[10] else None)
        sc(ws1, r, 12, row[11] or "", font=T_IN,  fill=rf, al=_R,
           fmt="#,##0.00" if row[11] else None)
        sc(ws1, r, 13, row[12] or "", font=T_OUT, fill=rf, al=_R,
           fmt="#,##0.00" if row[12] else None)
        t_qi += row[9]; t_vi += row[11]; t_vo += row[12]

    ws1.row_dimensions[5 + len(move_rows)].height = 6
    tot_row(ws1, 6 + len(move_rows),
            ["TOTAL", "", "", "", "", "", "", "", "", t_qi, "", t_vi, t_vo], 13)
    for ci, w in enumerate([12, 10, 12, 16, 22, 10, 8, 28, 8, 10, 16, 16, 20], 1):
    ...
    hdr_row(ws2, 4, [
        "Category", "Total Qty IN", "Value IN (₹)",
        "Total Qty OUT", "COGS / Value OUT (₹)",
        "Qty on Hand", "Closing Value (₹)"
    ])

    t_ci = 0.0; t_co = 0.0; t_vi2 = 0.0; t_vo2 = 0.0; t_oh = 0.0; t_tv = 0.0
    for i, (cat, v) in enumerate(sorted(cat_map.items())):
        r = 5 + i
        ws2.row_dimensions[r].height = 22
        cc  = CAT_CLR.get(cat, "777777")
        c_in  = float(v["in"]); c_out = float(v["out"])
        c_vi  = float(v["val_in"]); c_vo = float(v["val_out"])
        c_oh  = round(c_in - c_out, 4); c_cl = float(v["closing"])
        sc(ws2, r, 1, cat,  font=Font(name="Calibri", bold=True, color=cc, size=11), fill=F_CAT, al=_L)
        sc(ws2, r, 2, c_in, font=T_IN,   fill=F_CAT, al=_R, fmt="#,##0.000")
        sc(ws2, r, 3, c_vi, font=T_IN,   fill=F_CAT, al=_R, fmt="#,##0.00")
        sc(ws2, r, 4, c_out,font=T_OUT,  fill=F_CAT, al=_R, fmt="#,##0.000")
        sc(ws2, r, 5, c_vo, font=T_OUT,  fill=F_CAT, al=_R, fmt="#,##0.00")
        sc(ws2, r, 6, c_oh, font=T_NUM,  fill=F_CAT, al=_R, fmt="#,##0.000")
        sc(ws2, r, 7, c_cl, font=T_GOLD, fill=F_CAT, al=_R, fmt="#,##0.00")
        t_ci += c_in; t_co += c_out; t_vi2 += c_vi
        t_vo2 += c_vo; t_oh += c_oh; t_tv += c_cl

    ws2.row_dimensions[5 + len(cat_map)].height = 6
    tot_row(ws2, 6 + len(cat_map),
            ["TOTAL", t_ci, t_vi2, t_co, t_vo2, t_oh, t_tv], 7)
    for ci, w in enumerate([22, 14, 18, 14, 20, 14, 20], 1):
        ws2.column_dimensions[get_column_letter(ci)].width = w
    ws2.sheet_view.showGridLines = False

    # ════════════════════════════════════════════════════════════
    # SHEET 3 — FIFO Valuation (item-wise)
    # ════════════════════════════════════════════════════════════
    ws3 = wb.create_sheet("FIFO Valuation")
    title_block(ws3, "FIFO CLOSING STOCK VALUATION",
                f"As of: {today_str}  |  Method: First-In First-Out (FIFO)  |  Qty on Hand = IN − OUT", 9)
    hdr_row(ws3, 4, [
        "Category", "Purity", "Description", "Unit",
        "Total Qty IN", "Total Qty OUT", "Qty on Hand",
        "Avg Rate (₹)", "Closing Value (₹)"
    ])
