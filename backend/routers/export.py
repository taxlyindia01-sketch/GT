...
async def export_fifo_excel(
    ...
    """
    Comprehensive FIFO Excel — 3 sheets:
      Sheet 1 · Stock Movement   : every transaction row, invoice-wise (date, direction, invoice no, party, qty, rate, value)
      Sheet 2 · Category Summary : category-wise qty IN/OUT/on-hand and total value
      Sheet 3 · FIFO Valuation   : per-item avg rate & total value + grand total highlight box
    """
    ...
    # ── Build data ───────────────────────────────────────────────
    move_rows  = []      # [date, direction, txn_type, inv_no, party, cat, purity, desc, unit, qty, rate, val_in, val_out]
    cat_map    = {}      # category → [qty_in, qty_out, qty_on_hand, total_value]  (Decimals)
    val_rows   = []      # [cat, purity, desc, unit, qty_in, qty_out, qty_on_hand, avg_rate, total_value]
    ...
    for stock in stocks:
        ...
        txn_q = (
            ...
            .where(StockTransaction.stock_item_id == stock.id,
                   StockTransaction.txn_date      <= cutoff)
            .order_by(StockTransaction.txn_date, StockTransaction.id)
        )
        if from_date:
            txn_q = txn_q.where(StockTransaction.txn_date >= from_date)

        txns = (await db.execute(txn_q)).scalars().all()

        qty_in = Decimal("0"); qty_out = Decimal("0"); lots = []
        ...
        for t in txns:
            ...
            if t.invoice_id:
                if qty > 0:
                    si = await db.get(SupplierInvoice, t.invoice_id)
                    if si:
                        inv_no = si.invoice_no or f"SINV-{si.id}"
                        party  = (si.supplier_name or si.supplier_mobile) or "—"
                else:
                    ci = await db.get(Invoice, t.invoice_id)
                    if ci:
                        inv_no = ci.invoice_no or f"INV-{ci.id}"
                        party  = (ci.customer_name or ci.customer_mobile) or "—"
            ...
            move_rows.append([
                ...
                qa,
                rate if qty > 0 else 0.0,
                round(qa * rate, 2) if qty > 0 else 0.0,
                round(qa * rate, 2) if qty < 0 else 0.0,
            ])
            ...
            if qty > 0:
                ...
                if t.purchase_rate is not None and t.lot_remaining is not None:
                    lots.append({"qty_remaining": t.lot_remaining, "purchase_rate": t.purchase_rate})
            else:
                qty_out += abs(qty)

        val = fifo_valuation(lots)
        tv  = float(val["total_value"])
        val_rows.append([cat, stock.purity or "—", stock.description, unit,
                         float(qty_in), float(qty_out), float(stock.qty_on_hand),
                         float(val["avg_rate"]), tv])

        if cat not in cat_map:
            cat_map[cat] = [Decimal("0")] * 4
        cat_map[cat][0] += qty_in
        cat_map[cat][1] += qty_out
        cat_map[cat][2] += stock.qty_on_hand
        cat_map[cat][3] += Decimal(str(tv))
    ...
    T_BODY  = Font(name="Calibri", size=10, color="333333")
    ...
    _BDR = Border(left=Side(style="thin",color="E0E0E0"), right=Side(style="thin",color="E0E0E0"),
                  top=Side(style="thin",color="E0E0E0"),  bottom=Side(style="thin",color="E0E0E0"))
    CAT_CLR = {"Gold":"C8900A","Silver":"607D8B","Diamond":"7986CB"}

    def sc(ws, r, c, v, font=None, fill=None, al=None, fmt=None):
        """Safe cell write — skips MergedCell secondaries."""
        ...
        if hasattr(cell, 'font'):
            ...
            try: cell.border = _BDR
            except: pass
        return cell

    def title_block(ws, title, subtitle, ncols):
        ws.merge_cells(f"A1:{get_column_letter(ncols)}1")
        sc(ws,1,1,title, font=T_TTL, fill=F_NAV, al=_C)
        ws.row_dimensions[1].height = 30
        ws.merge_cells(f"A2:{get_column_letter(ncols)}2")
        sc(ws,2,1,subtitle, font=T_SUB, fill=F_ALT, al=_C)
    ...
    def tot_row(ws, rn, vals, ncols):
        ...
        for ci in range(1, ncols+1):
            ...
            sc(ws, rn, ci, v,
               ...
               fill=F_NAV,
               al=_L if ci==1 else _R,
               fmt="#,##0.00" if isinstance(v,float) and ci>4 else ("#,##0.000" if isinstance(v,float) else None))

    wb = openpyxl.Workbook()
    wb.remove(wb.active)

    # ════════════════════════════════════════════════════════════
    # SHEET 1 — Stock Movement (invoice-wise)
    # ════════════════════════════════════════════════════════════
    ws1 = wb.create_sheet("Stock Movement")
    title_block(ws1, "STOCK MOVEMENT REGISTER — INVOICE-WISE",
                f"Period: {period}  |  Generated: {today_str}", 13)
    h1 = ["Date","Direction","Txn Type","Invoice No","Party",
          "Category","Purity","Description","Unit",
          "Qty","Purchase Rate (₹)","Value IN (₹)","Value OUT (₹)"]
    hdr_row(ws1, 4, h1)

    t_val_in=0.0; t_val_out=0.0; t_qty=0.0
    for i, row in enumerate(move_rows):
        r = 5+i; ws1.row_dimensions[r].height = 16
        is_in = row[1]=="IN"
        rf = F_IN if is_in else F_OUT
        sc(ws1,r,1, row[0],  font=T_BODY, fill=rf, al=_C)
        sc(ws1,r,2, row[1],  font=T_IN if is_in else T_OUT, fill=rf, al=_C)
        sc(ws1,r,3, row[2],  font=T_BODY, fill=rf, al=_C)
        ...
        sc(ws1,r,11,row[10] or "", font=T_IN,  fill=rf, al=_R, fmt="#,##0.00" if row[10] else None)
        sc(ws1,r,12,row[11] or "", font=T_IN,  fill=rf, al=_R, fmt="#,##0.00" if row[11] else None)
        sc(ws1,r,13,row[12] or "", font=T_OUT, fill=rf, al=_R, fmt="#,##0.00" if row[12] else None)
        t_qty+=row[9]; t_val_in+=row[11]; t_val_out+=row[12]

    ws1.row_dimensions[5+len(move_rows)].height = 6   # spacer before total
    tot_row(ws1, 6+len(move_rows),
            ["TOTAL","","","","","","","","", t_qty,"", t_val_in, t_val_out], 13)
    for ci,w in enumerate([12,10,12,16,22,10,8,28,8,10,16,16,16],1):
    ...
    title_block(ws2, "STOCK SUMMARY — CATEGORY-WISE",
                f"As of: {today_str}  |  Period: {period}", 5)
    hdr_row(ws2, 4, ["Category","Total Qty IN","Total Qty OUT","Qty on Hand","Total Value (₹)"])

    t_ci=0.0; t_co=0.0; t_oh=0.0; t_tv=0.0
    for i,(cat,v) in enumerate(sorted(cat_map.items())):
        ...
        sc(ws2,r,2,float(v[0]),  font=T_IN,   fill=F_CAT, al=_R, fmt="#,##0.000")
        sc(ws2,r,3,float(v[1]),  font=T_OUT,  fill=F_CAT, al=_R, fmt="#,##0.000")
        sc(ws2,r,4,float(v[2]),  font=T_NUM,  fill=F_CAT, al=_R, fmt="#,##0.000")
        sc(ws2,r,5,float(v[3]),  font=T_GOLD, fill=F_CAT, al=_R, fmt="#,##0.00")
        t_ci+=float(v[0]); t_co+=float(v[1]); t_oh+=float(v[2]); t_tv+=float(v[3])

    ws2.row_dimensions[5+len(cat_map)].height = 6
    tot_row(ws2, 6+len(cat_map), ["TOTAL", t_ci, t_co, t_oh, t_tv], 5)
    for ci,w in enumerate([22,16,16,16,20],1):
    ...
    title_block(ws3, "FIFO STOCK VALUATION REPORT",
                f"As of: {today_str}  |  Method: First-In First-Out (FIFO)", 9)
    hdr_row(ws3, 4, ["Category","Purity","Description","Unit",
                     "Total Qty IN","Total Qty OUT","Qty on Hand","Avg Rate (₹)","Total Value (₹)"])
    ...
    t_qi3=sum(r[4] for r in val_rows); t_qo3=sum(r[5] for r in val_rows)
    t_oh3=sum(r[6] for r in val_rows)
    ws3.row_dimensions[5+len(val_rows)].height = 6
    tot_row(ws3, 6+len(val_rows),
            ["TOTAL","","","", t_qi3, t_qo3, t_oh3, "", grand_total], 9)

    # Grand total highlight
    gr = 8+len(val_rows)
    ws3.merge_cells(f"A{gr}:{get_column_letter(9)}{gr}")
    sc(ws3, gr, 1, f"TOTAL INVENTORY VALUE (FIFO):  ₹{grand_total:,.2f}",
       font=Font(name="Calibri",bold=True,color="FFD700",size=14),
       fill=F_NAV, al=_C)
    ws3.row_dimensions[gr].height = 28

    for ci,w in enumerate([12,8,28,8,14,14,14,16,18],1):
...
# ── TCS Excel — alias for Section 269ST Excel ─────────────────
# Frontend "269ST" tab Sheet Excel button calls /tcs-excel.
# This is an alias that calls the same logic as section-269st-excel.
...
async def export_tcs_excel(
    ...
    """Alias for /section-269st-excel — serves the TCS report tab Excel download."""
    return await export_section_269st_excel(
        from_date=from_date, to_date=to_date, payload=payload, db=db
    )
