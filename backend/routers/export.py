...
async def export_fifo_excel(
    ...
    Fixes applied (same as /api/reports/fifo):
      1. invoice_no / party populated for ALL txn types via pre-fetched lookup dicts
      2. Cancelled sales shown as REVERSAL rows (IN, "Cancelled Sale", value_in = COGS)
      3. Rate column for OUT rows = COGS / qty (FIFO avg), not 0
      4. FIFO batches built from ALL history (no from_date pre-filter)
      5. Edited invoices: only latest txn per (invoice_id, direction) kept
    """
    ...
    # ── Pre-fetch cancelled invoice ids ──────────────────────────
    ...
    cancelled_ids = set(row[0] for row in cancelled_r.fetchall())

    # ── Pre-fetch all invoices & supplier invoices ────────────────
    inv_r  = await db.execute(select(Invoice).where(Invoice.tenant_id == tenant_id))
    inv_map = {inv.id: inv for inv in inv_r.scalars().all()}

    sinv_r = await db.execute(select(SupplierInvoice).where(SupplierInvoice.tenant_id == tenant_id))
    sinv_map = {sinv.id: sinv for sinv in sinv_r.scalars().all()}
    ...
    for stock in stocks:
        ...
        # Load ALL txns up to cutoff — FIFO must start from day 1
        ...
        for t in txns:
            ...
            if t.invoice_id and qty > 0:
                ...
                if sinv:
                    ...
                    inv_no = f"SINV-{t.invoice_id}"; party = "—"
                ...
                if cinv:
                    ...
                    inv_no = f"INV-{t.invoice_id}"; party = "—"
            else:
                inv_no = label
                party  = t.reason or stock.description

            # Cancelled sale handling
            is_cancelled = (qty < 0 and t.invoice_id and t.invoice_id in cancelled_ids)
            if is_cancelled:
                in_window = (from_date is None or t.txn_date >= from_date)
                if in_window:
                    rev_cogs, _ = _fifo_out(
                        [dict(b) for b in fifo_batches], qty_abs
                    )
                    fifo_rate = float(rev_cogs / qty_abs) if qty_abs > 0 else 0.0
                    move_rows.append([
                        t.txn_date.isoformat(),
                        "IN",                  # reversal restores stock
                        "Cancelled Sale",
                        inv_no, party, cat,
                        stock.purity or "—",
                        ...
                        float(qty_abs),
                        round(fifo_rate, 2),
                        round(float(rev_cogs), 2),  # value_in
                        0.0,                         # value_out
                    ])
                continue
    ...
    F_CANC = PatternFill("solid", fgColor="FFF8E1")  # amber for cancelled reversal
    ...
    title_block(ws1, "INVOICE-WISE STOCK MOVEMENT REGISTER",
                f"Period: {period}  |  Rate (OUT) = FIFO avg consumed  |  Cancelled shown as IN reversal", 13)
    hdr_row(ws1, 4, [
    ...
    for i, row in enumerate(move_rows):
        ...
        is_in     = row[1] == "IN"
        is_canc   = row[2] == "Cancelled Sale"
        rf = F_CANC if is_canc else (F_IN if is_in else F_OUT)
        tf = T_CANC if is_canc else (T_IN if is_in else T_OUT)
        sc(ws1, r, 1,  row[0],  font=T_BODY, fill=rf, al=_C)
        sc(ws1, r, 2,  row[1],  font=tf,     fill=rf, al=_C)
        sc(ws1, r, 3,  row[2],  font=T_BODY if not is_canc else T_CANC, fill=rf, al=_C)
        ...
        sc(ws1, r, 11, row[10] or "", font=tf, fill=rf, al=_R,
           fmt="#,##0.00" if row[10] else None)
        sc(ws1, r, 12, row[11] or "", font=T_IN if row[11] else T_BODY, fill=rf, al=_R,
           fmt="#,##0.00" if row[11] else None)
        sc(ws1, r, 13, row[12] or "", font=T_OUT if row[12] else T_BODY, fill=rf, al=_R,
    ...
    for i, (cat, v) in enumerate(sorted(cat_map.items())):
        ...
        t_ci += ci_; t_co += co_; t_vi2 += vi_
        t_vo2 += vo_; t_oh += oh_; t_tv += cl_
    ...
    title_block(ws3, "FIFO CLOSING STOCK VALUATION",
                f"As of: {today_str}  |  Method: FIFO  |  Qty on Hand = IN − OUT (cancelled excluded)", 9)
    hdr_row(ws3, 4, [
        "Category", "Purity", "Description", "Unit",
