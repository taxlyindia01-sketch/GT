...
@router.get("/available-purities/")
async def available_purities(
    category: Optional[str] = None,
    payload:  dict          = Depends(get_current_user_payload),
    db:       AsyncSession  = Depends(get_db),
):
    """
    Return distinct purities (and available qty) for items with qty_on_hand > 0.
    Optionally filter by category. Used by New Invoice purity dropdown.
    """
    q = (
        select(StockItem)
        .where(
    ...
    if category:
        ...
        try:
            ...
            q = q.where(StockItem.category == cat_enum)
        except ValueError:
            pass
