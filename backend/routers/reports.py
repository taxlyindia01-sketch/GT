...
#  • Build FIFO layers from ALL IN transactions (from the very beginning)
#    regardless of from_date filter — only closing value is what matters.
#  • Replay every transaction in chronological order:
#      IN  → add new batch  { qty_remaining, purchase_rate }
#      OUT → consume oldest batches first (FIFO), record COGS
#  • Cancelled invoices: excluded by checking Invoice.status == 'cancelled'
#    (their StockTransactions are preserved but qty is either 0 or the
#     system adds a reversal txn — we handle both cases)
#  • Edited invoices: StockTransactions share the same invoice_id; we take
#    the LATEST txn per (stock_item_id, invoice_id, direction) by sorting by id.
#  • invoice_movements shown only within from_date..to_date window
#  • Category summary always uses final FIFO-replayed totals (no stale snapshots)
...
def _build_fifo_layers(batches: list, qty_out: "Decimal") -> tuple:
    ...
    Batches mutated in-place; exhausted batches removed.
    """
    from decimal import Decimal
    remaining = qty_out
    cogs = Decimal("0")
...
def _dedup_txns_for_item(all_txns: list) -> list:
    """
    For each (invoice_id, direction) pair keep only the LATEST StockTransaction
    (highest id). This correctly handles edited invoices where a new txn replaces
    the old one for the same invoice.
    Transactions without an invoice_id (opening/adjustment) are all kept.
    """
    seen: dict = {}
    no_inv = []
    for t in all_txns:
...
async def fifo_report(
    ...
    Rules implemented:
    1. Load ALL transactions from the beginning (never filter by from_date before building batches).
    2. Deduplicate per (invoice_id, direction) — edited invoices use latest txn only.
    3. Skip transactions linked to a CANCELLED invoice (status check on Invoice table).
    4. IN  → add FIFO batch  { qty_remaining, purchase_rate, date }
    5. OUT → consume FIFO batches oldest-first, record COGS = actual batch cost.
    6. Closing value = remaining batch quantities × their original purchase rates.
    7. invoice_movements displayed only within from_date..to_date window.
    8. qty_on_hand derived from replay (not stale DB field).
    """
    tenant_id = payload["tenant_id"]
    cutoff    = as_of or to_date or date.today()

    # Build cache of cancelled invoice ids so we can skip those transactions
    cancelled_inv_result = await db.execute(
        select(Invoice.id).where(
            Invoice.tenant_id == tenant_id,
            Invoice.status    == "cancelled",
        )
    )
    cancelled_invoice_ids = set(row[0] for row in cancelled_inv_result.fetchall())
    ...
    valuation_rows    = []
    invoice_movements = []   # populated only within date filter window
    category_map: dict = {}
    ...
    for stock in stocks:
        ...
        # Always fetch ALL transactions up to cutoff — FIFO must start from day 1
        ...
        # Deduplicate: edited invoices — keep only latest txn per (invoice_id, direction)
        txns = _dedup_txns_for_item(raw_txns)

        # ── FIFO REPLAY ────────────────────────────────────────────
        fifo_batches: list = []   # [{qty_remaining, purchase_rate, txn_date}]
        ...
        for t in txns:
            ...
            # Skip OUT transactions for cancelled invoices — the cancelled
            # sale never happened from a stock perspective
            if qty < 0 and t.invoice_id and t.invoice_id in cancelled_invoice_ids:
                continue

            # Resolve party / invoice number
            inv_no = "—"
            party  = "—"
            if t.invoice_id:
                if qty > 0:
        ...
        # Closing FIFO batches — whatever remains after all sales
        closing_batches = [
            {
        ...
        # qty_on_hand derived from replay (not stale DB field)
        qty_on_hand_calc = float(qty_in_total - qty_out_total)
        avg_rate = (
            float(closing_value / (qty_in_total - qty_out_total))
            if (qty_in_total - qty_out_total) > 0 else 0.0
        )
        ...
        valuation_rows.append({
            ...
            "qty_on_hand":     round(qty_on_hand_calc, 4),
