...
async def section_269st_via_tcs(
    ...
    Lists all cash transactions of ₹2,00,000 or more on a single invoice.

    Section 269ST of the Income Tax Act, 1961 prohibits receipt of cash of
    ₹2,00,000 or more in a single transaction.  Penalty u/s 271DA = amount received.

    Source: INVOICE grand_total where pay_mode='Cash' and grand_total >= 2,00,000.
    """
    tenant_id = payload["tenant_id"]
    threshold = Decimal("200000")

    # 269ST is checked at INVOICE level (cash invoice >= 2L is itself a violation)
    stmt = (
        select(Invoice)
        .where(
            Invoice.tenant_id == tenant_id,
    ...
    for inv in invoices:
        ...
        rows.append({
            ...
            "customer_mobile": inv.customer_mobile,
            "customer_name":   inv.customer_name,
            "customer_pan":    inv.customer_pan or (cust.pan if cust else None),
            "grand_total":     float(inv.grand_total),
            "cash_amount":     float(inv.grand_total),
            "penalty_risk":    float(inv.grand_total),
            "section":         "269ST",
            "notes":           inv.notes or "—",
        })
    ...
    return {
        ...
        "threshold":          float(threshold),
        "note": (
            "Section 269ST — No person shall receive ₹2,00,000 or more in cash "            "in a single transaction. Penalty u/s 271DA = amount received in violation."
        ),
    }
