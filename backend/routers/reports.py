...
async def cancelled_invoices_register(
    from_date: Optional[date] = Query(None),
    to_date:   Optional[date] = Query(None),
    payload:   dict           = Depends(get_current_user_payload),
    db:        AsyncSession   = Depends(get_db),
    ...
    Includes original invoice value, cancel date, and cancellation notes.
    ...
    for inv in invoices:
        rows.append({
            "invoice_no":       inv.invoice_no,
            "invoice_date":     inv.invoice_date.isoformat(),
            "cancelled_at":     inv.updated_at.isoformat() if inv.updated_at else None,
            "customer_mobile":  inv.customer_mobile,
            "customer_name":    inv.customer_name,
