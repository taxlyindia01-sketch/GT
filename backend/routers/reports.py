...
#  1. invoice_no / party correctly populated for ALL transaction types:
#       - purchase  → SupplierInvoice.invoice_no + supplier name
#       - sale      → Invoice.invoice_no + customer name
#       - opening   → "Opening Stock" + stock item description
#       - adjustment → "Adjustment" + reason text
#       - (no invoice_id transactions always had "—" — now show meaningful labels)
#  2. Cancelled sales invoice:
#       - FIFO OUT still skipped (correct — cancelled sale never consumed stock)
#       - Movement table shows a REVERSAL row: Direction=IN, Type="Cancelled Sale",
#         value_in = COGS that would have been consumed (= stock restored value)
#  3. Rate column for OUT rows = COGS ÷ qty (avg FIFO rate consumed), not 0
#  4. Dedup: edited invoices keep only latest txn per (invoice_id, direction)
#  5. qty_on_hand from live replay, not stale DB field
#  6. from_date never filters batch-building — only filters display window

def _build_fifo_layers(batches: list, qty_out: "Decimal") -> tuple:
    """
    Consume qty_out from FIFO batch list (oldest first).
    Returns (cogs: Decimal, updated_batches: list).
    """
    from decimal import Decimal
    remaining = qty_out
    cogs = Decimal("0")
...
def _dedup_txns(all_txns: list) -> list:
    """
    For each (invoice_id, direction) pair keep only the LATEST StockTransaction
    (highest id). Handles edited invoices correctly.
    Transactions without invoice_id (opening/adjustment) are all kept.
    """
    seen: dict = {}
    no_inv = []
    for t in all_txns:
        if t.invoice_id is None:
            ...
            key = (t.invoice_id, direction)
            if key not in seen or t.id > seen[key].id:
                seen[key] = t
    return no_inv + sorted(seen.values(), key=lambda x: (x.txn_date, x.id))


def _txn_type_label(txn_type_raw: str) -> str:
    """Human-readable label for txn_type enum values."""
    mapping = {
    ...
    return mapping.get(txn_type_raw.lower(), txn_type_raw.title())
...
async def fifo_report(
    ...
    """
    Correct FIFO Stock Valuation.

    invoice_no and party are populated for all transaction types:
      purchase   → SupplierInvoice.invoice_no, supplier name
      sale       → Invoice.invoice_no, customer name
      opening    → "Opening Stock", item description
      adjustment → "Adjustment", reason text

    Cancelled sales:
      The OUT transaction is excluded from FIFO calculation (correct — cancelled
      sale never consumed stock). The movement table shows a REVERSAL row
      (Direction=IN, Type="Cancelled Sale") so the restoration is visible.
    """
    tenant_id = payload["tenant_id"]
    cutoff    = as_of or to_date or date.today()

    # ── Pre-fetch all cancelled invoice ids ──────────────────────
    ...
    cancelled_ids = set(row[0] for row in cancelled_result.fetchall())

    # ── Pre-fetch ALL relevant invoices & supplier invoices ───────
    # Build look-up dicts to avoid per-txn N+1 queries
    inv_result = await db.execute(
        select(Invoice).where(Invoice.tenant_id == tenant_id)
    )
    inv_map: dict = {inv.id: inv for inv in inv_result.scalars().all()}

    sinv_result = await db.execute(
        select(SupplierInvoice).where(SupplierInvoice.tenant_id == tenant_id)
    )
    sinv_map: dict = {sinv.id: sinv for sinv in sinv_result.scalars().all()}
    ...
    for stock in stocks:
        ...
        # Always load ALL txns up to cutoff — never filter by from_date here
        ...
        # Deduplicate edited invoices
        txns = _dedup_txns(raw_txns)
        ...
        for t in txns:
            ...
            # ── Resolve invoice_no and party for every txn type ───
            if t.invoice_id and qty > 0:
                # Stock IN linked to supplier invoice
                sinv = sinv_map.get(t.invoice_id)
                if sinv:
                    inv_no = sinv.invoice_no or f"SINV-{sinv.id}"
                    party  = sinv.supplier_name or sinv.supplier_mobile or "—"
                else:
                    inv_no = f"SINV-{t.invoice_id}"
                    party  = "—"

            elif t.invoice_id and qty < 0:
                # Stock OUT linked to customer invoice
                cinv = inv_map.get(t.invoice_id)
                if cinv:
                    inv_no = cinv.invoice_no or f"INV-{cinv.id}"
                    party  = cinv.customer_name or cinv.customer_mobile or "—"
                else:
                    inv_no = f"INV-{t.invoice_id}"
                    party  = "—"

            else:
                # No invoice_id: opening stock or manual adjustment
                inv_no = txn_label             # e.g. "Opening Stock", "Adjustment"
                party  = t.reason or stock.description

            # ── CANCELLED SALE handling ───────────────────────────
            # If this OUT txn's invoice is cancelled:
            #   • Skip from FIFO batch consumption (stock was never taken)
            #   • But insert a REVERSAL row in movements (visible restoration)
            is_cancelled_sale = (qty < 0 and t.invoice_id and t.invoice_id in cancelled_ids)

            if is_cancelled_sale:
                # Show as a REVERSAL (IN) in the display window only
                in_window = (from_date is None or t.txn_date >= from_date)
                if in_window:
                    # COGS for the reversal = what the FIFO batches WOULD have given
                    # We compute it without actually consuming the batches (dry-run)
                    reversal_cogs, _ = _build_fifo_layers(
                        [dict(b) for b in fifo_batches], qty_abs  # copy, don't mutate
                    )
                    fifo_rate = float(reversal_cogs / qty_abs) if qty_abs > 0 else 0.0
                    invoice_movements.append({
                        "date":        t.txn_date.isoformat(),
                        "direction":   "IN",          # reversal restores stock
                        "txn_type":    "Cancelled Sale",
                        "invoice_no":  inv_no,
                        "party":       party,
                        "category":    cat,
                        ...
                        "value_in":    round(float(reversal_cogs), 2),
                        "value_out":   0.0,
                        "reason":      f"Cancelled — {inv_no}",
                    })
                continue   # skip from FIFO batch consumption entirely
            ...
            if qty > 0:
                ...
                value_in        = Decimal("0")
                # Average FIFO rate consumed this transaction
                fifo_rate = float(cogs / qty_abs) if qty_abs > 0 else 0.0

            # Append to movements only within the display date window
            in_window = (from_date is None or t.txn_date >= from_date)
            if in_window:
                invoice_movements.append({
                    "date":        t.txn_date.isoformat(),
                    ...
                    "unit":        unit,
                    "qty":         float(qty_abs),
                    # For IN: the actual purchase rate
                    # For OUT: the FIFO avg rate consumed (COGS/qty) — not 0
                    "rate":        round(fifo_rate, 2),
        ...
        # Closing FIFO batches after all deductions
        closing_batches = [
            {
        ...
        avg_rate        = (
            float(closing_value / qty_on_hand_val)
            if qty_on_hand_val > 0 else 0.0
        )
