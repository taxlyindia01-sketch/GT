...
async def record_advance(
    ...
    # Fetch customer name for storage (Issue 6 — improves list display)
    cust = await db.get(Customer, (body.customer_mobile, tenant_id))
    cust_name = cust.name if cust else "—"

    advance = Advance(
        tenant_id=tenant_id,
        customer_mobile=body.customer_mobile,
        customer_name=cust_name,   # stored for easy display
        amount=body.amount,
