...
async def _deduct_stock(
    ...
    for item in items:
        ...
        if not stock:
            continue  # no stock tracked for this category/purity — skip silently

        # Reduce qty_on_hand (allow going to 0 but not negative)
        new_qty = stock.qty_on_hand - item.qty
        stock.qty_on_hand = max(Decimal("0"), new_qty)
...
async def create_invoice(
    ...
    return {
        ...
        "tcs_applicable":   invoice.tcs_applicable,
