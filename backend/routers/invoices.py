...
async def cancel_invoice(
    ...
    await db.commit()
    # CRITICAL: After commit() SQLAlchemy expires all ORM object attributes.
    # Without refresh, accessing cn.cn_no / cn.id / cn.total_reversed triggers
    # a lazy load in async context → MissingGreenlet exception → HTTP 500.
    # This is why cancel was failing despite the transaction completing successfully.
    await db.refresh(cn)
