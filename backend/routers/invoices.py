...
async def create_invoice(
    ...
    # ── Auto-decrement stock (FIFO matching on category + purity) ──
    # For each invoice item, find the first StockItem with matching category
    # and purity (if set) that has qty_on_hand > 0, and decrement it.
    # If no matching stock item exists, we skip silently (stock and invoices
    # may be managed separately until full stock integration is in place).
    for item_obj, item_row in zip(body.items, item_rows):
        # Build query for matching stock item
        stock_q = (
            select(StockItem)
            .where(
        ...
        stock_q = stock_q.order_by(StockItem.id)   # FIFO: oldest lot first

        stock_result = await db.execute(stock_q)
        stock_item   = stock_result.scalars().first()

        if stock_item:
            qty_to_deduct = item_obj.qty
            # Clamp to available qty — don't go negative (enforced at adjust_stock level)
            deduct = min(qty_to_deduct, stock_item.qty_on_hand)
            if deduct > 0:
