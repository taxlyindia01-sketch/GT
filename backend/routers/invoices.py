...
async def cancel_invoice(
    ...
    # Read invoice_no BEFORE commit — db.commit() expires all ORM object
    # attributes. Accessing invoice.invoice_no after commit triggers an
    # implicit async lazy-load → MissingGreenlet exception → HTTP 500.
    invoice_no = invoice.invoice_no

    invoice.status = InvoiceStatus.cancelled
    await db.commit()
    return {"message": f"Invoice {invoice_no} cancelled", "invoice_no": invoice_no, "status": "cancelled"}
